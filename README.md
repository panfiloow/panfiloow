 👋 Привет, я Илья Панфилов | Python Backend Developer

[![Telegram](https://img.shields.io/badge/Telegram-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white)](https://t.me/bolomutik)
[![Email](https://img.shields.io/badge/Email-panfiloow@mail.ru-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:panfiloow@mail.ru)

## 🚀 Обо мне

```python
class BackendDeveloper:
    def __init__(self):
        self.name = "Илья Панфилов"
        self.birth_year = 2002
        self.location = "Россия"
        self.education = {
            "bachelor": "Петрозаводский государственный университет",
            "bachelor_major": "Прикладная математика", 
            "master": "Петрозаводский государственный университет",
            "master_major": "Анализ данных"
        }
        self.interests = [
            "Backend Development",
            "Data Engineering", 
            "System Design",
            "Machine Learning"
        ]
    
    def say_hello(self):
        return "Привет! Я backend-разработчик с фокусом на Python и современных веб-технологиях"

me = BackendDeveloper()
print(me.say_hello())
```

## 🛠 Технологический стек

### 🔧 Основные технологии
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white)
![SQLAlchemy](https://img.shields.io/badge/SQLAlchemy-FF6F00?style=for-the-badge&logo=python&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?style=for-the-badge&logo=postgresql&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white)
![Pytest](https://img.shields.io/badge/Pytest-0A9EDC?style=for-the-badge&logo=pytest&logoColor=white)

### 📚 Изучаю и планирую изучить
![Django](https://img.shields.io/badge/Django-092E20?style=for-the-badge&logo=django&logoColor=white)
![Celery](https://img.shields.io/badge/Celery-37814A?style=for-the-badge&logo=celery&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=for-the-badge&logo=kubernetes&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazon-aws&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white)
![RabbitMQ](https://img.shields.io/badge/RabbitMQ-FF6600?style=for-the-badge&logo=rabbitmq&logoColor=white)

### ⚙️ Инструменты и ОС
![Windows](https://img.shields.io/badge/Windows-0078D6?style=for-the-badge&logo=windows&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)

## 📊 GitHub Analytics

<div align="center">

### 📈 Статистика
![Stats](https://github-readme-stats.vercel.app/api?username=panfiloow&show_icons=true&theme=radical&hide_border=true&hide=stars,prs,issues,fork&count_private=true)
![Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=panfiloow&layout=compact&theme=radical&hide_border=true)

### 🔥 Активность
![Streak](https://github-readme-streak-stats.herokuapp.com/?user=panfiloow&theme=radical&hide_border=true)


</div>


## 🎓 Образование

### Петрозаводский государственный университет

#### 🎯 Магистратура 
**Направление:** Анализ данных  
**Период обучения:** 2024 – 2026  
**Статус:** Студент

#### 🎯 Бакалавриат
**Направление:** Прикладная математика  
**Период обучения:** 2020 – 2024  
**Статус:** Выпускник

---

*Сочетание фундаментального математического образования и современных методов анализа данных позволяет мне создавать эффективные и оптимизированные backend-решения.*


---
*Last updated: 2025-11-03 17:03:44 UTC
